...
from deploy.pipeline.funtction import predict_distance
from deploy.python.one_three_four_tools import lane_detection
...
from deploy.python.infer import Detector, DetectorPicoDet
...
from deploy.python.preprocess import decode_image, ShortSizeScale
from deploy.python.visualize import visualize_box_mask, visualize_attr, visualize_pose, visualize_action, visualize_vehicleplate, visualize_vehiclepress, visualize_lane, visualize_vehicle_retrograde
...
def ccw(A, B, C):  # 计算有A，B，C三点构成的向量CA，BA之间的关系，
...
class PipePredictor(object):
    ...
    def __init__(self, args, cfg, is_video=True, multi_camera=False):
        ...
        self.do_run_direction_detection = args.do_run_direction_detection
        self.illegal_parking = args.illegal_parking
    ...
    def predict_video(self, video_file, thread_idx=0):
        ...
        out_id_list = list()
        prev_center = dict()
        prev_result = []#存放上一帧的检测结果
        records = list()
        id_to_box = {}
        count = {}
        laneIDS = {}  # 存放当前帧车道信息
        leftlane1 = [(582, 328), (287, 719)]
        leftlane2 = [(614, 328), (462, 719)]
        rightlane1 = [(681, 328), (827, 719)]
        rightlane2 = [(714, 328), (1000, 719)]
        if self.do_entrance_counting or self.do_break_in_counting or self.illegal_parking_time != -1:
            ...
            if self.region_type == 'horizontal':
                entrance = [0, 300., width, 300]
            elif self.region_type == 'vertical':
                entrance = [width / 2, 0., width / 2, height]
            ##--region_polygon 0 120 1280 120 1280 600 0 600
        ...
        while (not framequeue.empty()):
            ...
            if self.modebase["idbased"] or self.modebase["skeletonbased"]:
                ...
                if frame_id >= 2 :
                # if frame_id >= 5 and frame_id % 5 == 0:#用于改变前后帧数 此时为相差5帧的连线

                    prev_result = (frame_id, boxes[0], scores[0], ids[0])
                    prev_boxes = prev_result[1]
                    prev_ids = prev_result[3]

                    for i in range(len(prev_boxes)):
                        id_to_box[prev_ids[i]] = prev_boxes[i]
                    # print('prev_result')
                    # print(prev_result)
                ...
                mot_boxes = mot_result[1]
                mot_ids = mot_result[3]

                for i in range(len(mot_boxes)):
                    mot_id = mot_ids[i]
                    if mot_id in id_to_box:  # 如果在 prev_result 中找到了相同 ID
                        prev_box = id_to_box[mot_id]
                        # # 计算框的左上角坐标
                        # prev_x, prev_y = prev_box[0], prev_box[1]
                        #
                        # x1, y1, x2, y2 = mot_boxes[i][0], mot_boxes[i][1] , mot_boxes[i][2], mot_boxes[i][3]
                        # w, h = int(mot_boxes[i][2] - mot_boxes[i][0]), int(mot_boxes[i][3] - mot_boxes[i][1])
                        # cx, cy = int(mot_boxes[i][0] + w / 2), int(mot_boxes[i][1] + h / 2)
                        # p0 = (cx, cy)
            ...
            if self.cfg['visual']:
                ...
                right3 = []
                #显示 检测结果代码加在下面
                for i, box in enumerate(mot_boxes):
                    mot_id = mot_ids[i]
                    x1, y1, x2, y2 = box[0], box[1], box[0]+box[2], box[1]+box[3]
                    w , h = int(x2-x1) , int(y2-y1)
                    cx, cy = int(box[0] + w / 2), int(box[1] + h / 2)
                    p0 = (cx, cy)
                    # cv2.circle(im, (int(x1), int(y1)), 5, (0, 0, 255), -1)  # 红色圆点表示起始点
                    # cv2.circle(im, (int(x2), int(y2)), 5, (0, 255, 0), -1)  # 绿色圆点表示结束点
                    # cv2.circle(im, (cx, cy), 5, (0, 255, 0), -1)  # 绿色圆点表示结束点
                    if cx > 648:
                        point1 = (cx + 0.16 * w, cy + 0.5 * h)
                        lane_point1 = (x1 + 0.333 * w, y1)
                        lane_point2 = (x1 + 0.666 * w, y2)

                    else:
                        point1 = (cx - 0.25 * w, cy + 0.5 * h)

                        lane_point1 = (x1 + 0.75 * w, y1)
                        lane_point2 = (x1 + 0.25 * w, y2)
                    if intersect(lane_point1, lane_point2, leftlane1[0], leftlane1[1]):
                        cv2.putText(im, 'turn', (int(x1), int(y2)), cv2.FONT_HERSHEY_PLAIN, 2,
                                    [0, 255, 255], 2)
                    if intersect(lane_point1, lane_point2, leftlane2[0], leftlane2[1]):
                        cv2.putText(im, 'turn', (int(x1), int(y2)), cv2.FONT_HERSHEY_PLAIN, 2,
                                    [0, 255, 255], 2)
                    if intersect(lane_point1, lane_point2, rightlane1[0], rightlane1[1]):
                        cv2.putText(im, 'turn', (int(x1), int(y2)), cv2.FONT_HERSHEY_PLAIN, 2,
                                    [0, 255, 255], 2)
                    if intersect(lane_point1, lane_point2, rightlane2[0], rightlane2[1]):
                        cv2.putText(im, 'turn', (int(x1), int(y2)), cv2.FONT_HERSHEY_PLAIN, 2,
                                    [0, 255, 255], 2)

                    rectangle1 = [(582, 328), (582, 328), (287, 719), (1, 719)]
                    result1 = lane_detection(point1, rectangle1)
                    if result1 == True:
                        text1 = 'L3'
                        cv2.putText(im, text1, (int(x2), int(y2)), cv2.FONT_HERSHEY_PLAIN, 2,
                                    [255, 255, 255], 2)
                        laneID = {id: 'L3'}
                        if lane_point2[1] < 685:
                            left3.append(lane_point2)  ### 添加车道信息 为后续划线做准备
                        if id not in laneIDS:
                            laneIDS.update(laneID)
                        # print(laneIDS)
                        # print(prelaneIDS)
                    rectangle2 = [(582, 328), (614, 328), (462, 719), (287, 719)]
                    result2 = lane_detection(point1, rectangle2)
                    if result2 == True:
                        text2 = 'L2'
                        cv2.putText(im, text2, (int(x2), int(y2)), cv2.FONT_HERSHEY_PLAIN, 2,
                                    [255, 255, 255], 2)
                        laneID = {id: 'L2'}
                        if lane_point2[1] < 685:
                            left2.append(lane_point2)  ### 添加车道信息 为后续划线做准备
                        if id not in laneIDS:
                            laneIDS.update(laneID)

                    rectangle4 = [(614, 328), (648, 328), (648, 719), (462, 719)]
                    result4 = lane_detection(point1, rectangle4)
                    if result4 == True:
                        text4 = 'L1'
                        cv2.putText(im, text4, (int(x2), int(y2)), cv2.FONT_HERSHEY_PLAIN, 2,
                                    [255, 255, 255], 2)
                        laneID = {id: 'L1'}
                        if lane_point2[1] < 685:
                            left1.append(lane_point2)  ### 添加车道信息 为后续划线做准备
                        if id not in laneIDS:
                            laneIDS.update(laneID)
                    rectangle3 = [(648, 328), (681, 328), (827, 719), (648, 719)]
                    result3 = lane_detection(point1, rectangle3)
                    if result3 == True:
                        text3 = 'R1'
                        cv2.putText(im, text3, (int(x2), int(y2)), cv2.FONT_HERSHEY_PLAIN, 2,
                                    [255, 255, 255], 2)
                        laneID = {id: 'R1'}
                        if id not in laneIDS:
                            laneIDS.update(laneID)
                        if lane_point2[1] > 338:
                            right1.append(lane_point2)
                    rectangle5 = [(681, 328), (714, 328), (1000, 719), (827, 719)]
                    result5 = lane_detection(point1, rectangle5)
                    if result5 == True:
                        text5 = 'R2'
                        cv2.putText(im, text5, (int(x2), int(y2)), cv2.FONT_HERSHEY_PLAIN, 2,
                                    [255, 255, 255], 2)
                        laneID = {id: 'R2'}
                        if lane_point2[1] > 338:
                            right2.append(lane_point2)

                        if id not in laneIDS:
                            laneIDS.update(laneID)

                    rectangle6 = [(714, 328), (753, 328), (1270, 719), (1000, 719)]
                    result6 = lane_detection(point1, rectangle6)
                    if result6 == True:
                        text6 = 'R3'
                        cv2.putText(im, text6, (int(x2), int(y2)), cv2.FONT_HERSHEY_PLAIN, 2, [255, 255, 255],
                                    2)
                        laneID = {id: 'R3'}
                        if lane_point2[1] > 338:
                            right3.append(lane_point2)
                        if id not in laneIDS:
                            laneIDS.update(laneID)

                    if mot_id in id_to_box:  # 如果在 prev_result 中找到了相同 ID
                        previous_box = id_to_box[mot_id]
                        # 计算框的左上角坐标
                        px1, py1, px2, py2 = previous_box[0], previous_box[1], previous_box[0] + previous_box[2], previous_box[1] + previous_box[3]
                        w, h = int(px2 - px1), int(py2 - py1)
                        cx_p, cy_p = int(box[0] + w / 2), int(box[1] + h / 2)
                        p1 = (cx_p, cy_p)
                        if cx > 648:
                            if p0[1] < p1[1]:
                                cv2.putText(im, 'F-run', (int(x1), int(y1)), cv2.FONT_HERSHEY_PLAIN, 2,
                                            [0, 255, 255], 2)
                        if cy_p == cy:
                            if mot_id not in count:
                                count[mot_id] = 0
                            elif mot_id in count:
                                if count[mot_id] >= 300:

                                    label_1 = "Stop!!"
                                    cv2.putText(im, label_1, (cx, cy + 4), cv2.FONT_HERSHEY_PLAIN, 2,
                                                [0, 255, 255], 2)
                                else:
                                    count[mot_id] = count[mot_id] + 1

                def draw_lane_lines(points, color):
                    if len(points) >= 2:
                        for n in range(len(points) - 1):
                            ...
                            if len(pt1) >= 2 and len(pt2) >= 2:
                                ...
                                rate = float(26 / 470)
                                distance = predict_distance(pt1, pt2, rate)
                                distance = abs(distance)
                                distance = round(distance, 1)
                                distance_str = str(distance)
                                text_size, _ = cv2.getTextSize(distance_str, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
                                text_x = int((pt1[0] + pt2[0] - text_size[0]) / 2)
                                text_y = int((pt1[1] + pt2[1] + text_size[1]) / 2)

                                cv2.putText(im, distance_str, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            (255, 0, 0), 2)

                sorted_left1 = sorted(left1, key=lambda p: p[1])
                draw_lane_lines(sorted_left1, (255, 255, 0))
                sorted_left2 = sorted(left2, key=lambda p: p[1])
                draw_lane_lines(sorted_left2, (255, 255, 0))
                sorted_left3 = sorted(left3, key=lambda p: p[1])
                draw_lane_lines(sorted_left3, (255, 255, 0))

                sorted_right1 = sorted(right1, key=lambda p: -p[1])
                draw_lane_lines(sorted_right1, (255, 0, 255))
                sorted_right2 = sorted(right2, key=lambda p: -p[1])
                print(sorted_right2)
                draw_lane_lines(sorted_right2, (255, 0, 255))
                sorted_right3 = sorted(right3, key=lambda p: -p[1])
                draw_lane_lines(sorted_right3, (255, 0, 255))
                ###同向车道划线
...
if __name__ == '__main__':
    ...
    parser = argsparser()
    parser.add_argument('--do_run_direction_detection', action='store_true', help='Whether to run direction detection')
    parser.add_argument('--illegal_parking', action='store_true', help='Whether to run direction detection')
